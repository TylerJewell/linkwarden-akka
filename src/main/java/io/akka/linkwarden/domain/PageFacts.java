 ...
 *
 * <p>SPEC-001 §4 C: the browser, the search engine and the file store are out of scope, so an
 * attempt is handed the facts a real one would have discovered rather than discovering them. Every
 * rule in §3 is about which step runs and what is written back, none of them about the bytes, so
 * the substitution stands exactly where the renderer does. The source probe at
