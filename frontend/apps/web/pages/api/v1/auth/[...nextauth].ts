  ...
  providers.push(
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        // Where the account lives. Looking the user up and comparing the password is the
        // server's decision, not this screen's, so it is asked for rather than repeated here.
        const response = await fetch(`${LINKWARDEN_API}/api/v1/session`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ username, password, sessionName: "Web" }),
        });
        const answer = await response.json();

        if (response.status === 401 && answer?.code === "EMAIL_NOT_VERIFIED") {
          throw Error("Email not verified.");
        }
        if (!response.ok) throw Error(answer?.response ?? "Invalid credentials.");

        // The token comes back with the account so that every later request this session makes
        // can present it; the session cookie carries it and nothing else knows it.
        const apiToken = answer.response.token as string;
        const me = await fetch(`${LINKWARDEN_API}/api/v1/users/me`, {
          headers: { Authorization: `Bearer ${apiToken}` },
        }).then((it) => it.json());

        return { id: me.response.id as number, apiToken };
      },
    })
  );
}
  ...
  return await NextAuth(req, res, {
    ...
    callbacks: {
      ...
      async jwt({ token, trigger, user }) {
        ...
        // The branch that used to stand here backfilled a username for an account that had
        // none. Where the account lives now, an account is never without one: R15 generates
        // it when the caller does not give one, so there is nothing to fix up on sign-in.
